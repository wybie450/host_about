...
嵌入式开发可以使用集成开发环境来提高开发效率。IDE 通常包括代码编辑器、编译器、调试器和版本控制工具的集成。常见的嵌入式开发 IDE 有：

**2.1** **STM32CubeIDE**

专为 STM32 系列微控制器设计的开发环境，集成了 STM32 的所有功能（如外设配置、代码生成等）。它内置了 `GCC` 工具链，并且支持调试、烧录、性能分析等功能。

**2.2** **Keil MDK**

Keil MDK 是为 ARM Cortex-M 系列微控制器设计的开发环境，提供强大的调试功能和优化编译器。它特别适用于性能要求高的嵌入式应用。

**2.3** **Eclipse + CDT**

Eclipse 是一个开源的 IDE，提供了强大的插件系统，可以支持多种编程语言和开发平台。通过安装 **CDT（C/C++ Development Tools）** 插件，Eclipse 可以成为一个强大的 C/C++ 开发环境。

**2.4** **Visual Studio Code（VS Code）**

VS Code 是一款轻量级的跨平台编辑器，支持多种编程语言。通过安装插件，可以用它进行嵌入式开发，如 ARM、ESP32、STM32 等平台。

**2.5** **IAR Embedded Workbench**

IAR 提供了针对嵌入式开发的集成开发环境，支持多种架构（如 ARM、Renesas 等）。它包括高效的编译器、调试器以及性能分析工具，适用于工业和商业应用。
...
- **JTAG（Joint Test Action Group）**：一种硬件调试接口，用于连接目标设备和调试工具，支持内存查看、寄存器操作、断点设置等调试功能。
- **SWD（Serial Wire Debug）**：是 JTAG 的一种简化版本，常用于 ARM Cortex 系列处理器的调试。

常见的调试器有：

- **ST-Link**：STMicroelectronics 提供的调试工具，支持 STM32 微控制器的调试。
- **J-Link**：由 Segger 提供的调试器，支持多种微控制器的调试。
- **CMSIS-DAP**：ARM 提供的标准调试协议，可以与多种调试器兼容。
...
在多任务的实时操作系统（RTOS）中，调试会更复杂。可以使用 **FreeRTOS Trace** 或 **Segger SystemView** 等工具来分析 RTOS 中的任务调度、堆栈使用情况等。

### 4. **烧录工具**

烧录工具用于将编译后的固件（如 `.bin` 或 `.elf` 文件）写入目标设备的存储器中。常见的烧录工具有：

**4.1** **ST-Link**

ST-Link 是 STMicroelectronics 提供的调试和烧录工具，支持 STM32 系列微控制器。它不仅可以进行调试，还可以直接将固件烧录到设备中。

**4.2** **J-Link**

J-Link 是 Segger 提供的调试器，也支持烧录功能，支持多种微控制器平台。

**4.3** **USBASP**

USBASP 是一个常见的用于烧录 AVR 系列微控制器的工具。它支持 SPI 协议，可以通过 USB 接口进行编程。

**4.4** **OpenOCD**

OpenOCD 是一个开源的硬件调试工具，它支持通过 JTAG/SWD 接口进行调试和烧录。可以与 GDB 配合使用进行调试，也可以独立进行烧录。
...
SVN 是集中式版本控制系统，虽然 Git 更为流行，但 SVN 仍然在一些嵌入式团队中使用。

**5.3** **GitLab/CircleCI/Jenkins**

这些持续集成工具可以帮助自动化构建、测试和部署嵌入式应用程序。通过集成 Git 代码库和构建系统，能够提高开发效率和代码质量。

### 6. **调试与性能分析工具**

**6.1** **Valgrind**

Valgrind 是一个用于检测内存泄漏、内存错误等的调试工具。虽然它更多用于通用 Linux 编程，但也适用于嵌入式 Linux 开发。

**6.2** **GProf**

GProf 是一个性能分析工具，可以帮助你识别程序中的性能瓶颈，优化代码。

**6.3** **OProfile**

OProfile 是一个 Linux 性能分析工具，能够提供关于系统各个部分的运行性能信息。

**6.4** **SystemTap**

SystemTap 是一个动态跟踪工具，能够实时监控嵌入式系统的性能，查看系统调用、内存使用情况等。 
...
- `tar -xvf Jetson_Linux_R35.4.1_aarch64.tbz2` 解压出 **Linux_for_Tegra** 目录。

- 然后必须再解压 **Sample RootFS** 到 `Linux_for_Tegra/rootfs/`。

```
sudo tar -xpf Tegra_Linux_Sample-Root-Filesystem_R35.4.1_aarch64.tbz2 -C rootfs/
```

- 最后执行 `sudo ./apply_binaries.sh`，这个脚本会把 CUDA、TensorRT、cuDNN 等 NVIDIA 二进制库覆盖进 rootfs（以及你 Jetson 设备上的系统）。

- 配置环境变量

- 验证 CUDA/TensorRT 是否恢复成功

Jetson 上自带的 CUDA 只包含 runtime，不含 `nvcc`，所以你无法执行 `nvcc --version`，这是正常的。



上面的依托把东西都安到镜像系统上了，它是刷写系统用的，ai死骗子

...
while true;do
 if ! pgrep -f '/home/nvidia/yolov5-dk-lk（26好的1版）/build/yolov5_det' > /dev/null; then
echo "shit!"
while [ ! -e /dev/ttyUSB_A ] || [ ! -e /dev/ttyUSB_B ]; do
	...
	gnome-terminal -- bash -c " 
	cd /home/nvidia/yolov5-dk-lk（26好的1版）/build
	./yolov5_det ../4060.engine s
 	"
 fi
 # echo "FUCK!"
...
Type=simple
User=nvidia
Environment=HOME=/home/nvicdia 
Environment=DISPLAY=:0
Environment=XAUTHORITY=/home/nvidia/.Xauthority
WorkingDirectory=/home/nvidia/StartUP
ExecStart=/bin/bash -c 'for f in /home/nvidia/StartUP/*.sh; do if [ -x "$f" ]; then "$f"; fi; done'
Restart=Always
RestartSec=3
...
```
sudo gedit /etc/udev/rules.d/99-serials.rules
```

```
SUBSYSTEM=="tty", KERNEL=="ttyCH341USB*", ATTRS{idVendor}=="1a86",KERNELS=="3-3.3:0", MODE="0666", SYMLINK+="ttyUSB_A"
SUBSYSTEM=="tty", KERNEL=="ttyCH341USB*", ATTRS{idVendor}=="1a86", KERNELS=="3-2.3:0", MODE="0666", SYMLINK+="ttyUSB_A""
```

```
udevadm info --query=path --name=/dev/ttyUSB0 # 查kernels
或者sudo dmesg -w | grep USB

sudo usermod -aG dialout 用户名 #给原指向描述符权限
...
在https://download.pytorch.org/whl/torch/里面找就行没有就

https://forums.developer.nvidia.com/t/pytorch-for-jetson/72048环境测试
...
- **下载dk驱动**
  ...
  sudo dpkg -i k4a-tools_1.4.1_arm64.deb #报错就改一下顺序
  ```

  https://blog.csdn.net/FL1623863129/article/details/148746638 dk驱动
...
- 如果是图为，前提是要把twsetup下的ch341.ko移到/lib/modules/drivers/usb/serials下面
